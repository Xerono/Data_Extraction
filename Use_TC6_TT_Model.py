...
def use(Inputs, Cust_Tok, Grenze, IsThisTrainingData):
    import os
    import pickle
    import time
    import sqlite3
    ...
    for (FPID, File, Par) in OriginalPars:
        if len(Tokenizer.tokenize(Par)) <= 500:
            ...
            for i in range(len(TokenizedPar)):
                ...
                if Full_Labels[i][2] == float(1):
                    if LabelsForPar[i][2] == float(1):
                        Coord_Correct += 1
                    if Full_Labels[i] == LabelsForPar[i]:
                        Full_Correct_Coords += 1
                    else:
                        Coord_False += 1
                else:
                    if LabelsForPar[i][2] == float(1):
    ...
    if Cust_Tok:
        ...
        Tokenizertype = "Normal"
    results_list = [(int(Cut_Par), int(CTN), int(Dele), int(DLabels), IsThisTrainingData, Tokenizertype, Treshold, Final_Time, Num_Of_All_Tokens,
                     Total_Corrects, Correct_Labels, RealCoordinate, Coord_Correct, Full_Correct_Coords, Coord_False, FalsePositiveCoords, AvgPerPar, AvgPerToken,
    ...
    Cur = Con.cursor()
    sql_command = "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='TC6_TT'"
    ...
    if res[0][0] == 0:
        sql_command = """
                ...
                Correct_Predicted_Coordclass INTEGER NOT NULL,
                Complete_Correct_Predicted_Coords INTEGER NOT NULL,
        ...
        Con.commit()
    sql_command = "INSERT INTO TC6_TT VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
