...
Files = os.listdir(Resultspath + "TC5_Loss")
psf = []
for file in Files:
    psf.append(file)
...
try:
    File = psf[int(ff)]
    Code = File.split(".")[0]
    loss_history = pickle.load(open(Resultspath + "TC5_Loss/" + File, "rb"))
    if list(Code)[3] == "1":
        closs_history = pickle.load(open(Resultspath + "TC5_Custom_Loss/" + File, "rb"))
    Paramsfile = "TC5_" + Code + "_Model/Parameters.pickle"
