...
if not os.path.exists(CurDir + "/Models/" + ModName + "/" + "Parameters.pickle"):
    ...
    for (oData, oDType) in OriginalData:
        for Par_Orig, Labels_Orig, (ParCrops, ParTextures, ParSoils, ParCords) in oData:
            ...
            for Label in Labels_Orig:
                if Label == No_Class or Label == Soils_Class:
                    Labels.append(Label)
                else:
            ...
            if oDType == "Train":
                ...
                if oDType == "Test":
                    ...
                    if oDType == "All":
                        ...
                        print(oDType)
                        input()
    with open(CurDir + "/Files/TCF_Soils_Training.pickle", "wb") as file:
        pickle.dump(TrainingData, file)
    with open(CurDir + "/Files/TCF_Soils_Test.pickle", "wb") as file:
        pickle.dump(TestData, file)
    with open(CurDir + "/Files/TCF_Soils_All.pickle", "wb") as file:
    ...
    with open(CurDir + "/Files/TCF_Soils_Training.pickle", "rb") as file:
        TrainingData = pickle.load(file)
    with open(CurDir + "/Files/TCF_Soils_Test.pickle", "rb") as file:
        TestData = pickle.load(file)
    with open(CurDir + "/Files/TCF_Soils_All.pickle", "rb") as file:
