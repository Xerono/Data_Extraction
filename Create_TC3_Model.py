...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        (SP, Labels) = Replace((Current_Par, CordList))
                        ...
                        for i in range(len(TNP)):
                            if i+len(Noisel) < len(TNP) and TNP[i:i+len(Noisel)] == Noisel:
                                for j in range(len(Noisel)):
                                    FullLabels[i+j] = LabelDict["Noise"]
            ...
            if Slices_With_Coords and random.choice([1,2,3,4]) == 1:
                ...
                for i in Deleted:
                    if i in range(len(TSPcoded)):
                        ...
                        del Attentionmask[i]
                for i in range(len(Labels)):
                    if Labels[i] not in [0, LabelDict["Noise"]]:
                        Labels[i] = LabelDict["Noise"]
    ...
    for Coord_To_Noise in Options:
        for Delete_Teilcoords in Options:
            for Custom_Loss in Options:
                ...
                mdl = "TC3_" + str(int(Cut_Par)) + str(int(Coord_To_Noise)) + str(int(Delete_Teilcoords)) + str(int(Custom_Loss))
                ModName =  mdl + "_Model_Coordinates/"
