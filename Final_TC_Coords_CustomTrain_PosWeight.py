...
ModName = "Coords_CustomTrain_PosWeight"
...
if not os.path.exists(CurDir + "/Models/" + ModName + "/" + "Parameters.pickle"):
    ...
    for (oData, oDType) in OriginalData:
        for Par_Orig, Labels_Orig, (ParCrops, ParTextures, ParSoils, ParCords) in oData:
            ...
            for Label in Labels_Orig:
                if Label == No_Class or Label == Coords_Class:
                    Labels.append(Label)
                else:
                    Labels.append(No_Class)
            if oDType == "Train":
                if Coords_Class in Labels:
                    for Label in Labels:
                        if Label == Coords_Class:
                            Weights[0] += 1
                        else:
                            Weights[1] += 1
                ...
                if oDType == "Test":
                    ...
                    if oDType == "All":
                        ...
                        print(oDType)
                        input()
    with open(CurDir + "/Files/TCF_" + ModName + "_Training.pickle", "wb") as file:
        pickle.dump(TrainingData, file)
    with open(CurDir + "/Files/TCF_" + ModName + "_Test.pickle", "wb") as file:
        pickle.dump(TestData, file)
    with open(CurDir + "/Files/TCF_" + ModName + "_All.pickle", "wb") as file:
    ...
    with open(CurDir + "/Files/TCF_" + ModName + "_Training.pickle", "rb") as file:
        TrainingData = pickle.load(file)
    with open(CurDir + "/Files/TCF_" + ModName + "_Test.pickle", "rb") as file:
        TestData = pickle.load(file)
    with open(CurDir + "/Files/TCF_" + ModName + "_All.pickle", "rb") as file:
...
for (Dater, DescriptorData) in Daten:
    for Treshold in Tresholds:
        ...
        for CountAna, (Par_Aus, Labels_Aus, (ParCrops, ParTextures, ParSoils, ParCords)) in enumerate(Dater):
            if CountAna%1000==0:
                print(ModName + " - " + DescriptorData + " - " + str(Treshold) + " | " + str(CountAna) + "/" + str(len(Dater)))
