...
def use(Inputs, Cust_Tok, Grenze):
    import os
    import pickle
    import time
    import sqlite3
    ...
    Model_Path = ModPath + "TC7_" + Clarifier + "_" + Inputs + "_Model/"
    ...
    results_list = [(int(Cut_Par), int(CTN), int(Dele), int(DLabels), Tokenizertype, Treshold, Final_Time, Num_Of_All_Tokens,
    ...
    Cur = Con.cursor()
    sql_command = "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='TC7'"
    ...
    if res[0][0] == 0:
        ...
        Con.commit()
    sql_command = "INSERT INTO TC7 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
