# Yet another different kind of coordinate generation
Storage = False
...
def create(Inputs, Cust_Tok):
    ...
    Custom_Loss = 0.1
    TestPercentage = 10
    mdl = "TC7_" + Clarifier + "_" + Code
    ModName = mdl + "_Model/"
    if not os.path.isdir(CurDir + "/Models/" + ModName):
        print("Starting " + mdl)
        ...
        import pickle
        PwC = []
        with open(CurDir + "/Files/TC7_Coords.pickle", "rb") as fff:
            All_Coordinates = pickle.load(fff)
        ...
        for (FPID, File, Par) in OriginalPars:
            ...
            Coords = []
            if len(Found_Coords)>0 and len(Tokenizer.tokenize(Par))<MaxLength:
     ...
     # Irrelevant, Noise, Coord, Grad, Min, Sek, Lat, Long
        ...
        while time.time() - Starttime < Stoptime:
            for batch in Training_Loader:
                ...
                optim.step()
                if Counter % 1000 == 0:
                    print(mdl + " with loss of " + str(round(lossnum, 6)) + " (" + str(Counter) + " steps, " + str(round(time.time() - Starttime, 2)) + "/" + str(Stoptime) + " seconds)")
                Counter += 1
                eetime = time.time()
        ...
        import pickle
        HistoryOutputPlace = CurDir + "/Results/TC7_Loss/"
        ErrorOutputPlace = CurDir + "/Results/TC7_Errors/" 
        ...
        print("Skipping " + mdl + ", folder exists")
